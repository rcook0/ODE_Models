...
function drawSeeds(){
  ...
  for(const s of seeds){const [X,Y]=worldToScreen(s.x,s.y);ctx.beginPath();ctx.arc(X,Y,3,0,2*Math.PI);ctx.fill();}
}
...
function integrateAndDraw(){
  ...
  try{
    ...
    let res=null;
    if(p.model==='lorenz') res=integrate_lorenz63(p.sigma,p.rho,p.beta,x0,y0,z0,0,p.T,p.dt);
    if(p.model==='rossler') res=integrate_rossler(p.a,p.b,p.c,x0,y0,z0,0,p.T,p.dt);
    const pts=[];
    for(let i=0;i<res.t.length;i++){
...
const liveUpdate = debounce(()=>{ if(el('live').checked) integrateAndDraw(); }, 220);
...
phase.addEventListener('click',(ev)=>{
  ...
  if(el('live').checked) integrateAndDraw(); else { redrawBase(); drawSeeds(); }
});
...
el('run').onclick=()=>integrateAndDraw();
...
el('exportSvg').onclick=()=>{
  const png=phase.toDataURL('image/png'); const w=phase.width,h=phase.height;
  const svg=`<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}"><image href="${png}" width="${w}" height="${h}"/></svg>`;
  downloadBlob(new Blob([svg],{type:'image/svg+xml'}),`ode_phase_${modelSel.value}.svg`);
};
